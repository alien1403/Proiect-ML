"""
Modelul prezentat este RandomForestClassifier
...
"""
    Definesc modelul RandomForestClassifier in care am setat pentru toate testele random_state = 20, n_jobs = -1 pentru a folosi toate core-urile disponibile de pe CPU,
max_depth = 20 pentru a nu ajunge la overfitting. Valorile pentru n_estimators si min_samples_leaf le-am determinat folosind
2 cicluri for in care am modificat valorile celor doua variabile si am luat rezultatele cele mai favorabile. La final am adaugat in variabila
class_weight un dictionar cu cheia numele fiecarui label si ca valoare va fi weight-ul calculat anterior. 

    Pentru a determina valorile lui n_estimators si min_samples_leaf:

for i in range(100,150):
    for j in range(100,150):
        classifier_rf = RandomForestClassifier(random_state= 20,n_jobs=-1,max_depth=20,n_estimators= i,min_samples_leaf = j,class_weight={1: one_weight, 0: zero_weight})
        classifier_rf.fit(reshaped_train_data,train_images_labels)

        predicted_validation = classifier_rf.predict(reshaped_validation_data)
        acc_validation = accuracy_score(validation_images_labels, predicted_validation)
        print(i,j)
        results.append((acc_validation,i,j))
        print("Accuracy validation: ", acc_validation)
        print("Precission score")
        print(precision_score(validation_images_labels, predicted_validation))
        print("Recall score")
        print(recall_score(validation_images_labels, predicted_validation))
        print("F1 score")
        print(f1_score(validation_images_labels, predicted_validation))  
results.sort(key= lambda x: x[0], reverse = True)
print(results[:3])       
[(0.69, 123, 100), (0.686, 144, 101), (0.6795, 112, 110)]
"""
rfc_model = RandomForestClassifier(random_state=20, n_jobs=-1, max_depth=20, n_estimators=112, min_samples_leaf = 110, class_weight={1: one_weight, 0: zero_weight})
"""
    Dau fit modelului cu datele de train si cu label-urile de train.
"""
rfc_model.fit(reshaped_train_data,train_images_labels)
"""
    Acum o sa prezicem label-urile datelor de validare pe care le stochez in predicted_validation.
"""
predicted_validation = rfc_model.predict(reshaped_validation_data)
"""
...
"""
predicted_test = rfc_model.predict(reshaped_test_data)
"""
...
"""
confusion_matrix_RFC = confusion_matrix(y_true=validation_images_labels, y_pred=predicted_validation)
...
confusion_matrix_RFC = confusion_matrix(validation_images_labels,predicted_validation)
figure, axes = plt.subplots()
axes.matshow(confusion_matrix_RFC, cmap=plt.cm.Purples)
for i in range(confusion_matrix_RFC.shape[0]):
    for j in range(confusion_matrix_RFC.shape[1]):
        axes.text(x=j,y=i,s=confusion_matrix_RFC[i,j],va='center',ha='center')
